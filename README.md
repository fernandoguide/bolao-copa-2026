...
| Testes    | Jest 29 (backend, 117 testes), Vitest (frontend, 66 testes)    |
...
| PATCH  | `/api/matches/:id/result`   | Registrar resultado (admin) |
...
// Empate no mata-mata (com pênaltis)
{ "homeScore": 1, "awayScore": 1, "homePenalty": 5, "awayPenalty": 3 }
```

Quando um resultado é salvo em jogo de mata-mata, o vencedor avança automaticamente para a próxima fase do chaveamento. Em caso de empate, os campos `homePenalty` e `awayPenalty` são obrigatórios e não podem ser iguais.
...
| ----- | ----- | ----------------------------- | -------------------- |
| 28/06 | 16h   | África do Sul × Canadá        | SoFi Stadium (EUA)   |
| 29/06 | 14h   | Brasil × Japão                | NRG Stadium (EUA)    |
| 29/06 | 17h30 | Alemanha × Paraguai           | Gillette Stadium     |
| 29/06 | 22h   | Holanda × Marrocos            | Estádio BBVA (MEX)   |
| 30/06 | 14h   | Costa do Marfim × Noruega     | AT&T Stadium (EUA)   |
| 30/06 | 18h   | França × Suécia               | MetLife Stadium      |
| 30/06 | 22h   | México × Equador              | Estádio Azteca (MEX) |
| 01/07 | 13h   | Inglaterra × RD Congo         | Mercedes-Benz Stad.  |
| 01/07 | 17h   | Bélgica × Senegal             | Lumen Field (EUA)    |
| 01/07 | 21h   | Estados Unidos × Bósnia       | Levi's Stadium       |
| 02/07 | 16h   | Espanha × Áustria             | SoFi Stadium (EUA)   |
| 02/07 | 20h   | Portugal × Croácia            | BMO Field (CAN)      |
| 03/07 | 0h    | Suíça × Argélia               | BC Place (CAN)       |
| 03/07 | 15h   | Austrália × Egito             | AT&T Stadium (EUA)   |
| 03/07 | 19h   | Argentina × Cabo Verde        | Hard Rock Stadium    |
| 03/07 | 22h30 | Colômbia × Gana               | Arrowhead Stadium    |
...
Jogo 76 ─┐                                          ├─ Jogo 102 ─┘
         ├─ Jogo 91 ─┐                              │
Jogo 78 ─┘            ├─ Jogo 99          Jogo 100 ─┘
Jogo 79 ─┐            │
         ├─ Jogo 92 ─┘                    Perdedores Semis → 3º lugar
Jogo 80 ─┘
Jogo 83 ─┐
         ├─ Jogo 93 ─┐
Jogo 84 ─┘            ├─ Jogo 98
...
| `npm run test`                                      | Roda todos os testes (117 testes) |
...
- `matches.security.spec.ts` — param/stage/result validation, penalty validation
...
5. Em jogos de mata-mata empatados, admin informa resultado dos pênaltis
6. Vencedor avança automaticamente para a próxima fase do chaveamento
7. Sistema calcula automaticamente os pontos de cada palpite
8. Ranking atualizado em tempo real na página de Leaderboard
